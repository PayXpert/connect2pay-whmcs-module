...
function payxpert_link($params) {
  ...
  if (isset($params['basecurrencyamount'])) {
	...
	$c2pClient->setCtrlRedirectURL( $params['returnurl'] );
	$c2pClient->setCtrlCallbackURL( $params['systemurl'] . '/modules/gateways/callback/payxpert.php' );
  ...
  $c2pClient->setCtrlCustomData ($baseCurrencyAmount . "|" . $md5);
	...
	if ($c2pClient->validate()) {
	  if ($c2pClient->prepareTransaction()) {
		...
		$code = '<form action="'. $url .'"><input type="submit" value="'. $params['langpaynow']. '" /></form>';
	  } else {
